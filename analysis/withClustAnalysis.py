...
def compute_best_worker_picking():
    ...
    #MV_clust["sample_num"] = MV_clust["sample"].apply(lambda x: int(x.split("rand")[-1]))
    #clust_df = pd.read_csv("spectral_clustering_all_hard_obj.csv")
    size_clust = pd.read_csv("actualNworkerInSampleForBestClustPicking.csv")
    ...
    best_clust_df = size_clust.loc[size_clust.groupby(["num_workers","sample_num","objid"])["actualNworkerInSample"].idxmax()]
    #best_clust_df = MV_clust.loc[MV_clust.groupby(["num_workers","sample_num","objid"])["actualNworkers"].idxmax()]
