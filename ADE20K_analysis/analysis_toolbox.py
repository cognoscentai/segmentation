...
def load_info(eliminate_self_intersection_bb=True):
    ...
    if eliminate_self_intersection_bb:
        for bb in bb_info.iterrows():
            bb=bb[1]
            xloc,yloc =  process_raw_locs([bb["x_locs"][1:-1],bb["y_locs"][1:-1]]) 
    ...
    #bb_info.to_csv("eliminated_self_intersection_bb.csv",index=None)
    ...
    if 'worker' in gtypes:
        ...
        if worker_id!=-1:
            ...
            for x,y in zip(bb_objects["x_locs"],bb_objects["y_locs"]):
                xloc,yloc = process_raw_locs([x[1:-1],y[1:-1]])
                if single:
